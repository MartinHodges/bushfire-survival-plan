...
class Questions(BaseModel):
    ...
    answers: Annotated[dict[str, str], operator.add] = Field(description="A list of questions and their answers", default_factory=dict)
...
class GraphState(BaseModel):
    ...
    messages: Optional[Annotated[list, add_messages]] = Field(description="A list of messages generated by the LLM.", default_factory=list, exclude=True)
...
def update_state_with_partial(current_state: GraphState, partial_update: dict):
    # Convert the current state to a dictionary using model_dump().
    current_dict = current_state.model_dump()
    
    # Merge the current state with the partial update.
    merged_state = {**current_dict, **partial_update}
    
    # Validate the merged state against the Pydantic model.
    # We don't need partial=True here because we're validating the full, merged state.
    return GraphState(**merged_state)